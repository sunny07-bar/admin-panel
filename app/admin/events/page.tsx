...
import { formatFloridaDateDDMMYYYY, formatFloridaTime, toFloridaTime } from "@/lib/utils/timezone";
              ...
              />
                ...
                {event.title}
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                      {(() => {
                        const formatted = formatFloridaDateDDMMYYYY(event.event_start);
                        // Debug log to verify what's being displayed
                        if (event.id === '586e03e0-d02d-438d-a4c3-43875acd83b5') {
                          console.log('[EventsList] Black Diamond date:', {
                            raw: event.event_start,
                            formatted: formatted
                          });
                        }
                        return formatted;
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col gap-2 items-end">
                        ...
                        {/* Action Button for Public Website - Only one based on action_button_type */}
                        <div className="flex gap-1 flex-wrap justify-end">
                          {(() => {
                            const eventStart = typeof event.event_start === 'string' ? toFloridaTime(parseISO(event.event_start)) : toFloridaTime(event.event_start);
                            const eventDate = format(eventStart, 'yyyy-MM-dd');
                            const eventTime = format(eventStart, 'HH:mm');
                            const websiteUrl = process.env.NEXT_PUBLIC_WEBSITE_URL || 'http://localhost:3000';
                            const actionType = event.action_button_type || 'reservation';
                            
                            if (actionType === 'reservation') {
                              return (
                                <a
                                  href={`${websiteUrl}/reservations?eventDate=${eventDate}&eventTime=${eventTime}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center px-2 py-1 text-xs font-medium rounded border border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800 dark:hover:bg-blue-900/30"
                                >
                                  Reserve
                                </a>
                              );
                            } else if (actionType === 'external_tickets' && event.ticket_link) {
                              return (
                                <a
                                  href={event.ticket_link}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center px-2 py-1 text-xs font-medium rounded border border-green-300 bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800 dark:hover:bg-green-900/30"
                                >
                                  Tickets
                                </a>
                              );
                            } else if (actionType === 'custom_tickets' && ((event.base_ticket_price !== null && event.base_ticket_price !== undefined) || (event.event_tickets && event.event_tickets.length > 0))) {
                              return (
                                <a
                                  href={`${websiteUrl}/events/${encodeURIComponent(event.slug)}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center px-2 py-1 text-xs font-medium rounded border border-purple-300 bg-purple-50 text-purple-700 hover:bg-purple-100 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-800 dark:hover:bg-purple-900/30"
                                >
                                  Get Ticket
                                </a>
                              );
