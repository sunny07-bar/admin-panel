...
export default function EditSpecialHoursPage() {
  ...
  const loadSpecialHours = async (id: string) => {
    ...
    try {
      ...
      // IMPORTANT: Do NOT use new Date() constructor as it will interpret DATE as UTC
      // and convert to local time, which can shift the date by one day
      let formattedDate = "";
      if (sh.date) {
        try {
          if (typeof sh.date === 'string') {
            // PostgreSQL DATE type is already in YYYY-MM-DD format
            // Just extract the date part (in case there's any time component)
            formattedDate = sh.date.split('T')[0].split(' ')[0];
            
            // Validate it's in YYYY-MM-DD format
            if (!/^\d{4}-\d{2}-\d{2}$/.test(formattedDate)) {
              console.warn('Date format is not YYYY-MM-DD:', sh.date);
              // If not in correct format, try to parse components directly
              // Don't use new Date() as it causes timezone issues
              const parts = formattedDate.match(/(\d{4})-(\d{2})-(\d{2})/);
              if (parts) {
                formattedDate = `${parts[1]}-${parts[2]}-${parts[3]}`;
              }
            }
          } else if (sh.date instanceof Date) {
            // If it's a Date object (shouldn't happen with DATE type, but handle it)
            // Use UTC methods to avoid timezone conversion
            const year = sh.date.getUTCFullYear();
            const month = String(sh.date.getUTCMonth() + 1).padStart(2, '0');
            const day = String(sh.date.getUTCDate()).padStart(2, '0');
            formattedDate = `${year}-${month}-${day}`;
          } else {
            // Fallback: try to extract YYYY-MM-DD from string representation
            formattedDate = String(sh.date).split('T')[0].split(' ')[0];
          }
        } catch (error) {
          console.error('Error formatting date:', error, sh.date);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let dateToSave = general.date;
      
      // Validate and normalize date format (YYYY-MM-DD)
      if (dateToSave) {
        // Remove any time component if present
        dateToSave = dateToSave.split('T')[0].split(' ')[0];
        
        // Validate format
        if (!/^\d{4}-\d{2}-\d{2}$/.test(dateToSave)) {
        ...
        const [year, month, day] = dateToSave.split('-').map(Number);
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <BackButton href="/admin/reservations/special-hours" />
          ...
          {activeTab === "general" && (
            <div className="space-y-6">
              ...
              {general.is_open && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="time_from">From Time (Florida Time)</Label>
                    <InputField
                      id="time_from"
                      type="time"
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      Time in Florida timezone (America/New_York)
                    </p>
                  </div>
                  <div>
                    <Label htmlFor="time_to">To Time (Florida Time)</Label>
                    <InputField
                      id="time_to"
                      type="time"
