...
export default function NewSpecialHoursPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data: specialHours, error: shError } = await supabase
        .from("special_hours")
        .insert({
          title: general.title,
          date: dateToSave,
      ...
      // Create seatings config (always create, even with defaults)
      const { error: seatingsError } = await supabase.from("special_hours_seatings").insert({
        special_hours_id: specialHours.id,
        interval_minutes: seatings.interval_minutes || 30,
        default_duration_minutes: seatings.default_duration_minutes || 90,
        duration_rule_type: seatings.duration_rule_type || 'fixed',
        duration_rules: Object.keys(seatings.duration_rules).length > 0 ? seatings.duration_rules : null,
      });
      
      if (seatingsError) {
        console.error('Error creating seatings:', seatingsError);
        // Continue anyway - seatings are optional
      }
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-dark">
        ...
        <div className="border-t border-gray-200 dark:border-gray-800 p-6 flex justify-end gap-4">
          ...
          <Button onClick={handleSave} disabled={loading}>
            {loading ? "Saving..." : "Save Special Hours"}
          </Button>
        </div>
      </div>
    </div>
