...
export default function SpecialHoursPage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Only update future reservations to avoid constraint violations
      if (relatedReservations && relatedReservations.length > 0) {
        ...
        // Filter to only future reservations (to avoid constraint violations)
        const { getFloridaNow } = require("@/lib/utils/timezone");
        const { fromZonedTime } = require("date-fns-tz");
        const { FLORIDA_TIMEZONE } = require("@/lib/utils/timezone");
        
        const floridaNow = getFloridaNow();
        const futureReservations = relatedReservations.filter((res: any) => {
          // Parse reservation date and time
          const [year, month, day] = res.reservation_date.split('-').map(Number);
          const [hour, minute] = res.reservation_time.split(':').map(Number);
          ...
          const { toFloridaTime } = require("@/lib/utils/timezone");
          const reservationDateTimeFlorida = toFloridaTime(reservationDateTimeUTC);
          
          // Only include future reservations
          return reservationDateTimeFlorida >= floridaNow;
        });
        
        console.log(`Found ${futureReservations.length} future reservations out of ${relatedReservations.length} total`);
        
        if (futureReservations.length > 0) {
          // Update each reservation individually to handle constraint errors gracefully
          let successCount = 0;
          let errorCount = 0;
          ...
          for (const reservation of futureReservations) {
            try {
              ...
              if (updateError) {
                // If constraint violation, skip this reservation (it's likely in the past)
                if (updateError.code === '23514' || updateError.message.includes('reservation_datetime_future')) {
                  console.warn(`Skipping reservation ${reservation.id} - constraint violation (likely past reservation)`);
                } else {
                  console.warn(`Failed to update reservation ${reservation.id}:`, updateError.message);
                }
                errorCount++;
              } else {
                successCount++;
              ...
              console.warn(`Error updating reservation ${reservation.id}:`, err.message || err);
              errorCount++;
            }
          }
          
          console.log(`Updated ${successCount} reservations, ${errorCount} skipped/failed`);
        } else {
          console.log("No future reservations to update - all reservations are in the past");
        }
        
        // Note: Past reservations will keep their special_hours_id reference
        // This is acceptable as they are historical records and won't block deletion
      }
                ...
                specialHours.map((sh) => (
                  <tr key={sh.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                    ...
                    <td className="px-6 py-4 text-theme-sm text-gray-600 dark:text-gray-400">
                      {formatFloridaDateDDMMYYYY(sh.date)}
                    </td>
                    <td className="px-6 py-4 text-theme-sm text-gray-600 dark:text-gray-400">
