...
import { formatFloridaTime, formatDateOnlyMMDDYYYY } from "@/lib/utils/timezone";
      ...
      if (newStatus === 'confirmed') {
        try {
          await fetch('/api/reservations/send-confirmation', {
            ...
            body: JSON.stringify({ reservationId: params.id }),
          });
        } catch (emailError) {
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-dark p-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-theme-sm text-gray-600 dark:text-gray-400">Reservation Number</p>
                <p className="font-semibold font-mono text-lg">{reservation.id.slice(0, 8).toUpperCase()}</p>
                <p className="text-theme-xs text-gray-500 dark:text-gray-500 mt-1">
                  This is the number sent to the customer in their confirmation email
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-semibold">
                  {formatDateOnlyMMDDYYYY(reservation.reservation_date)} at {convert24To12(reservation.reservation_time || '')}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                {reservation.prepayment_amount && (
                  <p className="text-theme-sm mt-2">
                    Prepayment Amount: ${parseFloat(reservation.prepayment_amount).toFixed(2)}
                  </p>
                )}
                {reservation.payment_method && (
                  <p className="text-theme-sm mt-1 text-gray-600 dark:text-gray-400">
                    Method: {reservation.payment_method.toUpperCase()}
                  </p>
                )}
                {reservation.payment_transaction_id && (
                  <p className="text-theme-xs mt-1 text-gray-500 dark:text-gray-500 font-mono">
                    Transaction: {reservation.payment_transaction_id.slice(0, 20)}...
                  </p>
                )}
                {reservation.payment_date && (
                  <p className="text-theme-xs mt-1 text-gray-500 dark:text-gray-500">
                    Paid: {formatFloridaTime(reservation.payment_date, 'MM-dd-yyyy h:mm a')}
                  </p>
                )}
              </div>
